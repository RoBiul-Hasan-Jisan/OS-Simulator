  ...
  useEffect(() => {
    ...
    const simulationStep = async (): Promise<void> => {
      if (simulationState.executingProcess) {
        ...
        const continueExecution = async (): Promise<void> => {
      ...
      const arrivedProcesses = pendingProcesses.filter(p => parseInt(p.arrivalTime.toString()) <= currentTime);
      
      if (arrivedProcesses.length === 0) {
        const nextArrival = Math.min(...pendingProcesses.map(p => parseInt(p.arrivalTime.toString())));
        setAnimatingTimeJump(true);
      ...
      for (let i = 1; i < arrivedProcesses.length; i++) {
        ...
        if (parseInt(processToCompare.priority.toString()) < parseInt(highestPriorityProcess.priority.toString())) {
      ...
      const startTime = Math.max(currentTime, parseInt(highestPriorityProcess.arrivalTime.toString()));
      const endTime = startTime + parseInt(highestPriorityProcess.burstTime.toString());
