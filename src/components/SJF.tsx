...
const SJF = ({ processes }: SJFProps) => {
  ...
  const addNewProcesses = (newProcesses: Process[]) => {
    const filteredNewProcesses = newProcesses.filter(newProcess => 
      parseInt(newProcess.arrivalTime?.toString() || "0") >= currentTime &&
  ...
  useEffect(() => {
    ...
    const simulationStep = async () => {
      if (simulationState.executingProcess) {
        ...
        setTimeJumpTarget(simulationState.processEndTime || 0);
        
        const continueExecution = async () => {
          for (let t = currentTime + 1; t <= (simulationState.processEndTime || 0); t++) {
      ...
      const arrivedProcesses = pendingProcesses.filter(p => parseInt(p.arrivalTime?.toString() || "0") <= currentTime);
      
      if (arrivedProcesses.length === 0) {
        const nextArrival = Math.min(...pendingProcesses.map(p => parseInt(p.arrivalTime?.toString() || "0")));
        setAnimatingTimeJump(true);
      ...
      for (let i = 1; i < arrivedProcesses.length; i++) {
        ...
        if (parseInt(processToCompare.burstTime.toString()) < parseInt(minProcess.burstTime.toString())) {
      ...
      const startTime = Math.max(currentTime, parseInt(minProcess.arrivalTime?.toString() || "0"));
          ...
          <div className="relative">
            ...
            <div className="flex h-20 mb-12 relative">
              {ganttChart.map((p, index) => {
                const prevEndTime = index > 0 ? (ganttChart[index - 1].endTime || 0) : 0;
                const idleTime = (p.startTime || 0) - prevEndTime;
                const totalTime = ganttChart[ganttChart.length - 1].endTime || 0;
                const idleWidth = (idleTime / totalTime) * 100;
                const processWidth = (((p.endTime || 0) - (p.startTime || 0)) / totalTime) * 100;
                
                return (
                  <div key={`${p.name}-${index}`} className="flex h-full group">
                    {idleTime > 0 && (
                      <div 
                    ...
                    <div className="relative h-full px-1">
                      ...
                      >
                        <div className="flex flex-col items-center">
                          ...
                          <span className="text-xs">{(p.endTime || 0) - (p.startTime || 0)}u</span>
                        </div>
                      </div>
        ...
        <div className="w-full bg-black p-4 rounded-lg border border-white mt-8">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full bg-black">
              ...
              <tbody>
                {ganttChart.map(p => {
                  const turnaroundTime = (p.endTime || 0) - parseInt(p.arrivalTime?.toString() || "0");
                  const waitingTime = (p.startTime || 0) - parseInt(p.arrivalTime?.toString() || "0");
                ...
                {ganttChart.length > 0 && (
                  <tr className="bg-gray-900 font-semibold">
                    ...
                    <td className="py-2 px-4 border border-white text-center">
                      {(ganttChart.reduce((sum, p) => sum + ((p.endTime || 0) - parseInt(p.arrivalTime?.toString() || "0")), 0) / ganttChart.length).toFixed(2)}
                    </td>
                    <td className="py-2 px-4 border border-white text-center">
                      {(ganttChart.reduce((sum, p) => sum + ((p.startTime || 0) - parseInt(p.arrivalTime?.toString() || "0")), 0) / ganttChart.length).toFixed(2)}
                    </td>
                  </tr>
                )}
              </tbody>
