...
const RoundRobin = ({ processes, timeQuantum = 2 }: RoundRobinProps) => {
  ...
  const startSimulation = () => {
    if (!simulationState.initialized) {
      ...
      const processesWithRemaining = processes.map(p => ({
        ...
        burstTime: parseInt(p.burstTime.toString()),
        arrivalTime: parseInt(p.arrivalTime?.toString() || "0")
      }));
      
      // Initialize remaining time
      const initialRemaining: Record<string, number> = {};
      processesWithRemaining.forEach(p => {
        initialRemaining[p.name] = parseInt(p.burstTime.toString());
      });
  ...
  useEffect(() => {
    ...
    const simulationStep = async () => {
      ...
      const newArrivals = pendingProcesses.filter(p => (p.arrivalTime || 0) <= currentTime);
      ...
      if (newArrivals.length > 0) {
        ...
        // Update queues
        setPendingProcesses(prev => prev.filter(p => (p.arrivalTime || 0) > currentTime));
      ...
      if (!currentProcess && readyQueue.length > 0) {
        ...
        const nextArrivalTime = Math.min(...pendingProcesses.map(p => p.arrivalTime || 0));
                ...
                <div className="grid grid-cols-1 gap-1 mt-2 text-xs">
                  ...
                  <div className="w-full bg-gray-800 h-1 rounded-full overflow-hidden mt-1">
                    <div 
                      className="h-full"
                      style={{
                        width: `${((parseInt(p.burstTime.toString()) - (p.remainingTime || 0)) / parseInt(p.burstTime.toString())) * 100}%`,
